...
public class DataAccessUserExists {
	...
	public DataAccessUserExists(boolean initializeMode)  {
...
//			setResult(4, "0 goles");
	...
	private void anadirApuesta(String userName, int i, float parseFloat, String empate) {
		db.getTransaction().begin();
	...
	private float addDinero(String  username, float money, boolean b) {
		db.getTransaction().begin();
	...
	private void anadirPronostico(int i, Vector<String> vec, Vector<Float> vec2) {
		db.getTransaction().begin();
		Question q=db.find(Question.class, i);
