...
public class CloseEventGUI extends JFrame {
	...
	private final JButton btnCloseQuery = new JButton("Cerrar Pregunta"); //$NON-NLS-1$ //$NON-NLS-2$
	private final JLabel lblNewLabel_1 = new JLabel("Escribe el resultado de la pregunta: "); //$NON-NLS-1$ //$NON-NLS-2$
	private final JTextField textField = new JTextField();
	private final JButton btnCloseEvent = new JButton("Cerrar Event");
	private final JLabel errorLbl = new JLabel(""); //$NON-NLS-1$ //$NON-NLS-2$

	public CloseEventGUI(String userName)
	{
		textField.setText(""); 
		textField.setBounds(302, 218, 193, 19);
	...
	private void jbInit(String userName) throws Exception
	{
		...
		jButtonClose.setBounds(new Rectangle(149, 423, 130, 30));
...
//		scrollPanePronosticos.setViewportView(tablePronosticos);
		...
		btnCloseQuery.setEnabled(true);
		btnCloseQuery.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if(textField.getText().length()>0) {
					int i=tableQueries.getSelectedRow();
					int q=(int)tableModelQueries.getValueAt(i,0);
					boolean esta= false;
					Vector<String>vec= facade.getQuestionByNum(q).getPronosticos();
					int num =0;
					while(!esta && num < vec.size()) {
						if(textField.getText().compareTo(vec.get(num))==0) {
							esta= true;
						}else {
							num++;
						}
					}if(esta && facade.getQuestionByNum(q).getResult()==null) {
						facade.setResult(q, textField.getText());
						String respuesta= textField.getText();
						Vector<Apuesta>vecApuesta= facade.getApuestasQuestion(q);
						int num2=0;
						while(num2<vecApuesta.size()) {
							Apuesta apuesta= vecApuesta.get(num2);
							if(apuesta.getPronostico().compareTo(respuesta)==0) {
								User user= facade.getUserByName(apuesta.getName());
								float porc= facade.getPorcentajesPronosticosPregunta(q).get(num);
								float money= apuesta.getCantidad()+(apuesta.getCantidad() * porc);
								facade.anadirDinero(user.getUserName(), money, true);
							}
							num2++;
						}
						errorLbl.setForeground(Color.GREEN);
						errorLbl.setText("El resultado se ha a�adido correctamente.");
						...
						errorLbl.setForeground(Color.RED);
						errorLbl.setText("No existe ningun pronostico as� para esta pregunta.");
					}
				}else {
					errorLbl.setForeground(Color.RED);
					errorLbl.setText("Introduce el resultado correspondiente a la pregunta.");
				}
			}
		});
		btnCloseQuery.setBounds(new Rectangle(157, 411, 130, 30));
		btnCloseQuery.setBounds(342, 423, 130, 30);		
		
		getContentPane().add(btnCloseQuery);
		lblNewLabel_1.setBounds(40, 224, 207, 13);
		...
		btnCloseEvent.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				int num=0;
				while(cerradas && num< tableQueries.getColumnModel().getColumnCount()) {
					int q=(int)tableModelQueries.getValueAt(num,0);
					if(facade.getQuestionByNum(q).getResult()!= null || facade.getQuestionByNum(q).getApuestas()==null ) {
						num++;
					}else {
						cerradas=false;
					}
		...
		btnCloseEvent.setBounds(new Rectangle(157, 411, 130, 30));
		btnCloseEvent.setBounds(461, 423, 130, 30);
		
		getContentPane().add(btnCloseEvent);
		*/
		errorLbl.setBounds(78, 397, 476, 13);
