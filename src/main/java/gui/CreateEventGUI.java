...
//	private JTextField jTextFieldPrice = new JTextField();
	...
	private static int evNumber=21;

	public CreateEventGUI(Vector<domain.Event> v, String userName) {
		try {
			jbInit(v, userName);
		} catch (Exception e) {
			e.printStackTrace();
	...
	private void jbInit(Vector<domain.Event> v, String userName) throws Exception {
		...
		jLabelQuery.setBounds(new Rectangle(10, 211, 90, 20));
...
//				else {
					...
					BLFacade facade = MainGUI.getBusinessLogic();
					boolean ok= true;
					Vector<Event>eventos= facade.getEvents(jCalendar.getDate());
					for(int i=0; i<eventos.size();i++) {
						if(eventos.get(i).getDescription().equals(inputQuery)) {
							ok=false;
							jLabelMsg.setText("Event already exist.");
							break;
						}
					}
					if(ok) {
						evNumber= evNumber+1;
						int day= calendarAct.get(Calendar.DAY_OF_MONTH);
						int month=calendarAct.get(Calendar.MONTH);
						int year=calendarAct.get(Calendar.YEAR);
						event= facade.createEvent(evNumber, inputQuery, UtilDate.newDate(year, month, day));
