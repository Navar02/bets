		...
		createUserJButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) /*throws NullPointerException*/{
				...
				if(pwd.equals(pwd1) && pwd.length()>0 && pwd1.length()>0 && !facade.userExist(usuario) && nTarjeta.length()==16 && !facade.tarjetaExist(nTarjeta)) {
					String user= facade.anadirUsuario(usuario, pwd, nTarjeta, correo); // Mandar el nombre del usuario y la contrase�a
					System.out.println("El usuario: "+ user + " se ha a�adido.");
					...
					errorLabel.setText("El n�mero de la tarjete debe contener 16 numeros.");
				}else if(facade.tarjetaExist(nTarjeta)) {
					errorLabel.setText("Ya existe un user con dicha tarjeta.");
					errorLabel2.setText("Introduce tu verdadero n�mero de tarjeta por favor.");
				}else {
